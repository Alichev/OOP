...
public class ConsoleView {
        ...
        for (int i = 0; i < Program.GANG_SIZE; i++) {
            if (Program.whiteSide.get(i).getPosition().isEquals(position)) {
                if (Program.whiteSide.get(i).hp == 0)
                    str = "|" + AnsiColors.ANSI_RED + Program.whiteSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
                else {
                    str = "|" + AnsiColors.ANSI_GREEN + Program.whiteSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
                    alive = true;
                }
            }
            if (Program.darkSide.get(i).getPosition().isEquals(position) && !alive) {
                if (Program.darkSide.get(i).hp == 0)
                    str = "|" + AnsiColors.ANSI_RED + Program.darkSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
                else
                    str = "|" + AnsiColors.ANSI_BLUE + Program.darkSide.get(i).getName().toUpperCase().charAt(0)
                            + AnsiColors.ANSI_RESET;
            }
        }
        return str;
    ...
    private static String PrintInfo(int npcIndex) {
        ...
        if (Program.whiteSide.get(npcIndex).hp == 0)
            ...
            str += "     " + AnsiColors.ANSI_GREEN + Program.whiteSide.get(npcIndex).getInfo() + AnsiColors.ANSI_RESET;
        if (Program.darkSide.get(npcIndex).hp == 0)
            str += "     " + AnsiColors.ANSI_RED + Program.darkSide.get(npcIndex).getInfo() + AnsiColors.ANSI_RESET;
        else
            str += "     " + AnsiColors.ANSI_BLUE + Program.darkSide.get(npcIndex).getInfo() + AnsiColors.ANSI_RESET;
